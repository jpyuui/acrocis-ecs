          ...
          conditions = {
            ...
            http_headers = {
              from_cloudfront = {
                ...
                values = [
                  data.aws_ssm_parameter.cloudfront_header_value_for_alb.value,
                ]
              }
            }
          }
...
# Pipelineで利用するcodestar connection
resource "aws_codestarconnections_connection" "for_pipeline" {
  name          = "${var.service_name}-connection-${var.env}"
...
module "pipeline_for_web" {
  ...
  env                          = var.env
  region                       = var.region
  connection_arn               = aws_codestarconnections_connection.for_pipeline.arn
  repository_owner_name        = "framgia"
  repository_name              = "xxx-dxpj-web"
  trigger_branch_name          = "release"
  buildspec_file_name          = "buildspec.prod.yml"
...
module "ecs_web_app" {
  ...
  task_config = {
    ...
    container_definitions = templatefile(
      ...
      {
        image_uri                    = module.ecr_for_web.image_uri
...
# ===========================
module "ecr_for_subscriber" {
  source               = "../../modules/ecr"
  name                 = "${var.service_name}-subscriber-${var.env}"
  image_tag_mutability = "MUTABLE"
}
...
# codebuildを特定のsubnetに配置する際にsgの指定が必須なので適当なものを用意
module "migrate_sg" {
  source        = "../../modules/sg"
  name          = "${var.service_name}_migrate_sg_${var.env}"
  vpc_id        = module.network.vpc_id
  ingress_rules = {}
}

module "pipeline_with_migrate_for_subscriber" {
  source                              = "../../modules/pipeline/deploy_with_migrate"
  name                                = "subscriber"
  env                                 = var.env
  region                              = var.region
  connection_arn                      = aws_codestarconnections_connection.for_pipeline.arn
  repository_owner_name               = "framgia"
  repository_name                     = "xxx-dxpj-subscriber"
  trigger_branch_name                 = "release"
  buildspec_file_name                 = "buildspec.prod.yml"
  buildspec_migrate_file_name         = "buildspec.migrate.yml"
  migrate_db_host                     = module.rds_cluster_main.host
  migrate_db_name                     = module.rds_cluster_main.name
  ssm_db_username_name                = module.db_username.name
  ssm_db_password_name                = module.db_password.name
  vpc_id                              = module.network.vpc_id
  migrate_execute_subnet_ids          = values(module.network.private_subnets)[*].id
  migrate_execute_sg_id               = module.migrate_sg.id
  deploy_cluster_name                 = aws_ecs_cluster.main.name
  deploy_service_name                 = module.ecs_subscriber_app.service_name
  notification_emails                 = var.developper_emails
  code_pipeline_iam_role_arn          = module.iam_role_for_code_pipeline.arn
  code_build_iam_role_arn             = module.iam_role_for_code_build.arn
  code_build_for_migrate_iam_role_arn = module.iam_role_for_migrate_code_build.arn
  notifier_lambda_iam_role_arn        = module.iam_role_for_lambda_for_error_notifier.arn
}
...
module "ecs_subscriber_app" {
  ...
  task_config = {
    ...
    container_definitions = templatefile(
      ...
      {
        image_uri                    = module.ecr_for_subscriber.image_uri
...
module "ecs_migrate_task" {
  ...
  container_definitions = templatefile(
    ...
    {
      image_uri                 = module.ecr_for_subscriber.image_uri # migrateはsubscriberのimageに内包
      app_port                  = 80
      sqs_url                   = module.sqs.url
      database_host             = module.rds_cluster_main.host
      database_name             = module.rds_cluster_main.name
